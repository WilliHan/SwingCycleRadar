> ⚠️ **SUPERSEDED (2026-08-08)** — 이 v1.0은 KRX 연동 방식(Open API Marketplace 가정), Pullback 액션 로직 부재, Dow/Cycle 정성적 서술, ADX Gate 미정의 분기, trade_plans 유니크 제약 부재 등의 문제로 개정되었다. **구현은 반드시 [`SwingCycle_Radar_Source_Level_Design_v1.1.md`](/home/mhhan/projects/wt/SwingCycle/docs/SwingCycle_Radar_Source_Level_Design_v1.1.md)를 기준으로 진행한다.** 이 파일은 변경 이력 대조용으로만 보존한다.

# SwingCycle Radar

## KRX 기반 절친종목 사이클·진입점수 시스템 소스 레벨 상세 설계서 v1.0

* 작성 기준일: 2026-08-08
* 목적: 절친 종목(universe)을 미리 선정해두고, **좋은 종목을 다시 고르는 것이 아니라 좋은 진입 시점과 사이클을 자동 판정**한다.
* 1차 데이터 소스: **KRX Data Marketplace OPEN API**
* 2차 백업 데이터 소스: **pykrx**
* 운용 주기: 일봉 EOD 기준
...
* 절친 종목등 관리가 필요한 항목은 supabase db 에 관리한다.
...
```text
                    +-----------------------------+
                    |  config/friend\_universe.yml |
                    +--------------+--------------+
                                   |
                                   v
+---------------+      +----------------------+      +---------------------+
| KRX Open API  | ---> | KRX Primary Adapter  | ---> | Raw OHLCV Store     |
+---------------+      +----------------------+      +---------------------+
                             | failure                       |
                             v                               v
                       +-----------+                 +---------------------+
...
KRX OPEN API는 Data Marketplace 인증키 신청 및 API별 활용 신청/승인 후 사용한다. 공식 서비스는 JSON/XML을 제공하며, 주식 일별매매정보는 2010년 이후 데이터를 제공하는 것으로 안내되어 있다.

v1 구현 원칙:

1. **KRX OPEN API가 primary**.
2. pykrx는 primary 호출 실패 또는 누락 데이터 보완용 fallback.
3. 동일 영업일/종목에 KRX와 pykrx가 모두 있으면 KRX를 authoritative source로 저장.
4. 원본 응답(raw payload)의 hash와 source를 기록하여 데이터 계보(lineage)를 보존.
5. KRX API ID/endpoint는 코드에 박지 말고 `markets.yml`로 분리.
...
```dotenv
KRX\_AUTH\_KEY=
KRX\_BASE\_URL=https://data-dbg.krx.co.kr/svc/apis
DB\_PATH=./data/swingcycle.db
LOG\_LEVEL=INFO
HTTP\_TIMEOUT\_SEC=20
HTTP\_MAX\_RETRIES=3
```

> 실제 KRX base URL/API ID는 승인받은 개발명세서에 맞춰 config에서 교체할 수 있어야 한다.

## 6.3 config/markets.yml 예시

```yaml
krx:
  base\_url: ${KRX\_BASE\_URL}
  auth\_header: AUTH\_KEY
  services:
    kospi\_daily:
      api\_id: TBD\_FROM\_KRX\_SPEC
      path: TBD\_FROM\_KRX\_SPEC
    kosdaq\_daily:
      api\_id: TBD\_FROM\_KRX\_SPEC
      path: TBD\_FROM\_KRX\_SPEC
  request:
    ...
    date\_format: "%Y%m%d"
  response:
    output\_key\_candidates: \[OutBlock\_1, output, data]
...
`src/swingcycle/data/krx\_client.py`
...
class KRXClient:
    def \_\_init\_\_(self, base\_url: str, auth\_key: str, timeout: float = 20.0): ...

    def fetch\_daily\_market(self, market: str, trade\_date: date) -> KRXResponse:
        """시장 전체 일별매매정보를 1회 호출해 반환한다."""

    def \_request\_json(self, path: str, params: dict\[str, str]) -> dict\[str, Any]: ...
    def \_extract\_rows(self, payload: dict\[str, Any]) -> list\[dict\[str, Any]]: ...
```

### 요청 정책

* retry: 429/5xx/network timeout에 exponential backoff `1s, 2s, 4s`.
* 4xx 인증오류는 즉시 실패 처리.
...
`src/swingcycle/data/pykrx\_client.py`

```python
class PykrxClient:
    def fetch\_symbol\_ohlcv(self, symbol: str, start: date, end: date) -> pd.DataFrame: ...
```

fallback 조건:

```python
if krx\_request\_failed:
    fallback(symbols\_missing, trade\_date)
elif krx\_rows\_missing\_friend\_symbols:
    fallback(missing\_symbols, trade\_date)
```

저장 시 `source = 'KRX' | 'PYKRX'`를 반드시 기록한다.
...
REVERSAL\_CANDIDATE:
  마지막 의미 있는 LH 고점 돌파 OR LL 갱신 실패 + HL 후보

UPTREND:
  확정된 HH + HL 조합

RANGE:
  HH/LL 갱신 실패가 반복되고 pivot 간 범위가 축소/평행
```

> 실제 전략상 \*\*다우가 핵심 게이트\*\*이므로 이 모듈은 회귀테스트를 가장 많이 확보한다.
...
if core < 70:
    ...
    action = READY if gate != BLOCK else WAIT
elif core >= 80 and gate == PASS:
    action = ENTRY
elif core >= 80 and gate == CAUTION:
    action = READY
else:
    action = WAIT
```
